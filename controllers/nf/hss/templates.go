...
func renderConfigFiles(log logr.Logger, values configurationTemplateValues) ([]string, error) {
	...
	xfiles := [4]string{"templates/hss/acl.conf", "templates/hss/hss.conf", "templates/hss/hss.json", "templates/hss/oss.json"}
...
func renderScriptFiles(log logr.Logger, values configurationTemplateValues) ([]string, error) {
	var buffer bytes.Buffer
	var theArray []string
	xfiles := [2]string{"templates/hss/_hss-bootstrap.sh.tpl", "templates/hss/_hss-run.sh.tpl"}
