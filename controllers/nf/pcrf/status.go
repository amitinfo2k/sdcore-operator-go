...
func createNfDeploymentStatus(deployment *appsv1.Deployment, pcrfDeployment *v1alpha1.PCRFDeployment) (nephiov1alpha1.NFDeploymentStatus, bool) {
	nfDeploymentStatus := nephiov1alpha1.NFDeploymentStatus{
		ObservedGeneration: int32(deployment.Generation),
		Conditions:         pcrfDeployment.Status.Conditions,
	}

	// Return initial status if there are no status update happened for the PCRFdeployment
	if len(pcrfDeployment.Status.Conditions) == 0 {
	...
	} else if (len(deployment.Status.Conditions) == 0) && (len(pcrfDeployment.Status.Conditions) > 0) {
	...
	lastAmfDeploymentCondition := pcrfDeployment.Status.Conditions[len(pcrfDeployment.Status.Conditions)-1]
