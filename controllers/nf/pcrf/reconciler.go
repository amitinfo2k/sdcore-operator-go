...
// Reconciles a PCRFDeployment resource
type PCRFDeploymentReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// Sets up the controller with the Manager
func (r *PCRFDeploymentReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(new(v1alpha1.PCRFDeployment)).
		Owns(new(appsv1.Deployment)).
		Owns(new(apiv1.ConfigMap)).
		Complete(r)
}

// +kubebuilder:rbac:groups=workload.nephio.org,resources=pcrfdeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=workload.nephio.org,resources=pcrfdeployments/status,verbs=get;update;patch
...
// the PCRFDeployment object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.14.1/pkg/reconcile
func (r *PCRFDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx).WithValues("PCRFDeployment", req.NamespacedName)

	pcrfDeployment := new(v1alpha1.PCRFDeployment)
	err := r.Client.Get(ctx, req.NamespacedName, pcrfDeployment)
	if err != nil {
		if k8serrors.IsNotFound(err) {
			log.Info("PCRFDeployment resource not found, ignoring sibecausence object must be deleted")
			return reconcile.Result{}, nil
		}
		log.Error(err, "Failed to get PCRFDeployment")
	...
	configMapName := "pcrf-configs"
	ScriptConfigMapName := "pcrf-scripts"
	...
	serviceFound := false
	serviceName := pcrfDeployment.Name
	...
	deploymentName := pcrfDeployment.Name
	...
	if deploymentFound {
		...
		// Updating PCRFDeployment status. On the first sets the first Condition to Reconciling.
		...
		if deployment.DeletionTimestamp == nil {
			if err := r.syncStatus(ctx, deployment, pcrfDeployment); err != nil {
	...
	if configMap, err := createConfigMap(log, pcrfDeployment); err == nil {
		if !configMapFound {
			log.Info("Creating ConfigMap", "ConfigMap.namespace", configMap.Namespace, "ConfigMap.name", configMap.Name)

			// Set the controller reference, specifying that PCRFDeployment controling underlying deployment
			if err := ctrl.SetControllerReference(pcrfDeployment, configMap, r.Scheme); err != nil {
	...
	if configMap, err := createScriptConfigMap(log, pcrfDeployment); err == nil {
	...
	if !serviceFound {
		service := createService(pcrfDeployment)

		log.Info("Creating PCRFDeployment service", "Service.namespace", service.Namespace, "Service.name", service.Name)

		// Set the controller reference, specifying that PCRFDeployment controling underlying deployment
		if err := ctrl.SetControllerReference(pcrfDeployment, service, r.Scheme); err != nil {
			log.Error(err, "Got error while setting Owner reference on PCRF service", "Service.namespace", service.Namespace, "Service.name", service.Name)
		}
	...
	if deployment, err := createDeployment(log, configMapVersion, pcrfDeployment); err == nil {
		//if deployment, err := createDeployment(log, configMapVersion, pcrfDeployment); err == nil {
		if !deploymentFound {
			// Only create Deployment in case all required NADs are present. Otherwise Requeue in 10 sec.
			//if ok := controllers.ValidateNetworkAttachmentDefinitions(ctx, r.Client, log, pcrfDeployment.Kind, deployment); ok {
			// Set the controller reference, specifying that PCRFDeployment controls the underlying Deployment
			if err := ctrl.SetControllerReference(pcrfDeployment, deployment, r.Scheme); err != nil {
...
func (r *PCRFDeploymentReconciler) syncStatus(ctx context.Context, deployment *appsv1.Deployment, pcrfDeployment *v1alpha1.PCRFDeployment) error {
	if nfDeploymentStatus, update := createNfDeploymentStatus(deployment, pcrfDeployment); update {
		pcrfDeployment = pcrfDeployment.DeepCopy()
		pcrfDeployment.Status.NFDeploymentStatus = nfDeploymentStatus
		return r.Status().Update(ctx, pcrfDeployment)
	} else {
		return nil
	}
}
