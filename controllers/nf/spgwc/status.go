...
func createNfDeploymentStatus(deployment *appsv1.Deployment, spgwcDeployment *nephiov1alpha1.NFDeployment) (nephiov1alpha1.NFDeploymentStatus, bool) {
	nfDeploymentStatus := nephiov1alpha1.NFDeploymentStatus{
		ObservedGeneration: int32(deployment.Generation),
		Conditions:         spgwcDeployment.Status.Conditions,
	}

	// Return initial status if there are no status update happened for the SPGWCdeployment
	if len(spgwcDeployment.Status.Conditions) == 0 {
		nfDeploymentStatus.Conditions = append(nfDeploymentStatus.Conditions, metav1.Condition{
			...
			Message:            "SPGWCDeployment pod(s) is(are) starting.",
			LastTransitionTime: metav1.Now(),
		})

		return nfDeploymentStatus, true
	} else if (len(deployment.Status.Conditions) == 0) && (len(spgwcDeployment.Status.Conditions) > 0) {
	...
	lastAmfDeploymentCondition := spgwcDeployment.Status.Conditions[len(spgwcDeployment.Status.Conditions)-1]

	// Deployemnt and SPGWCDeployment have different names for processing state, hence we check if one is processing another is reconciling, then state is equal
	...
	case appsv1.DeploymentAvailable:
		nfDeploymentStatus.Conditions = append(nfDeploymentStatus.Conditions, metav1.Condition{
			...
			Message:            "SPGWCDeployment pods are available.",
			LastTransitionTime: metav1.Now(),
		})
	...
	case appsv1.DeploymentReplicaFailure:
		nfDeploymentStatus.Conditions = append(nfDeploymentStatus.Conditions, metav1.Condition{
			...
			Status:             metav1.ConditionFalse,
			Reason:             "MinimumReplicasNotAvailable",
			Message:            "SPGWCDeployment pod(s) is(are) failing.",
			LastTransitionTime: metav1.Now(),
		})
	}
